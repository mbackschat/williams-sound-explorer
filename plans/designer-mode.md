...
| 5 | **GWAVE editor** — full editing of all bytes the GWAVE kernel reads (SVTAB record + GWVTAB waveforms + GFRTAB pitch patterns), in 3 steps | 🚧 Step 1 ✅ (SVTAB override-in-place); Steps 2–3 pending (waveform/pattern canvases) |
...
2. **Editable waveform canvas (GWVTAB bytes).** Selected slot's current WAVE# loads the corresponding GWVTAB entry's bytes (8 / 16 / 72) onto a click-to-draw canvas; edits write into the custom ROM's GWVTAB at the entry's existing position. **Lengths don't change** (no new waveforms in this step), so byte offsets stay valid — no pointer rebase. Sharing caveat surfaced in UI: *"This waveform is also used by command(s) X, Y — your edits affect those sounds too."* No copy-on-edit. Extend `engine/gwaveEdit.ts` (`readWaveform` / `patchWaveform` + a "shared by" lookup) and `engine/customRom.ts`.
3. **Editable pitch pattern canvas (GFRTAB bytes).** Selected slot's pattern (length from SVTAB byte 5, offset from byte 6) loads the corresponding GFRTAB bytes onto a second canvas; edits write into the custom ROM's GFRTAB at the existing offset+length. **Length doesn't change**, so SVTAB byte-6 offsets stay valid. Sharing caveat same as Step 2. Extend `engine/gwaveEdit.ts` (`readPattern` / `patchPattern`) and `engine/customRom.ts`.
...
- **Adding new waveforms (extending GWVTAB).** **Feasible, low risk, ~minimal rewrite of the current solution.** WAVE# is a *position index* (GWLD walks the table by `length+1` per entry), so appending `[length, …bytes]` to GWVTAB Just Works at the kernel level. Space comes from **relocating GWVTAB into the free ROM region we already use for VARI's VVECT extension** (over RADIO/ORGAN data) and repointing the single `LDX #GWVTAB` immediate in `GWLD2`/`GWLD3` — one-instruction patch per game, same shape as VARI's mask widen. Schema impact: `slot.kind: "gwave"` gains an optional `addedWaveforms?: Uint8Array[]` and slots can reference an idx ≥ 7. No reshape of the existing slot logic. Natural "Step 4" if pursued.
