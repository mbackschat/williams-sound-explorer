...
This covers the **VARI**, **GWAVE**, and **LFSR** engines. VARI slots are *new sounds* added at command codes `$1D`+ (the engine's dispatcher has spare bits we widen); GWAVE and LFSR slots *override* an existing command's parameters in place (their dispatchers are hardcoded — no spare codes — so the model differs). LFSR is the noise family: **LITE** (lightning, `$11`), **TURBO** (turbo burst, `$14`), **APPEAR** (enemy-appear descent, `$15`), and Robotron's **LAUNCH** (`$39`).
...
2. **The list opens with the game's sound bank already loaded.** *New Project → Defender* pre-populates the item list with every editable command — 13 GWAVE rows (`$01 HBDV` … `$0D ED17`), 3 LFSR rows (`$11 LITE` / `$14 TURBO` / `$15 APPEAR`), and 3 VARI rows (`$1D SAW` / `$1E FOSHIT` / `$1F QUASAR`), 19 in total. Stargate is the same; Robotron has 13 GWAVE rows + 4 LFSR rows (the extra one is `$39 LAUNCH`) and no VARI. Each row carries a **● dot indicator** before its code:
...
3. **Add new sounds beyond the base game's set** — the **+ New VARI** and **Copy VARI:** controls still add **extra** VARI slots at the next free code (`$20`, `$21`, …):
   ...
   - VARI rows show yellow `$XX VARI`; GWAVE rows show purple `$XX GWAVE`; LFSR rows show teal `$XX LFSR`.
...
**Per-engine score:** Studio 5, WSED today **3** → after Phase 8 = 4 (one gap: RADIO) → after Phase 9 = **5 (parity on Defender)**.
