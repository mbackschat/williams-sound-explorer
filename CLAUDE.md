...
- **Sound Designer mode (internals)**: [`docs/designer_implementation.md`](docs/designer_implementation.md) (impl state); roadmap + decision log in [`plans/designer-mode.md`](plans/designer-mode.md) — the separate Design mode for editing VARI + GWAVE sounds (LFSR + FNOISE + RADIO editors planned)
...
| Sound Designer mode (VARI + GWAVE editors, .bin roundtrip): module map, decisions, record references, recipe schema | `docs/designer_implementation.md` |
...
There is also a separate **Sound Designer mode** (top-level Explore ↔ Design toggle): fork the game's sound bank — opens with every editable command pre-loaded, edit any **VARI** or **GWAVE** sound's parameter record, audition/A/B, save as a JSON recipe (no ROM bytes) or download a runnable custom `.bin` for MAME / a real cabinet, upload it back to keep editing. **LFSR + FNOISE + RADIO** editors are planned next (Phases 7 + 8 + 9). Its implementation state lives in `docs/designer_implementation.md` (the designer analog of the implementation doc below); the Explore UI is unchanged by it.
...
- **Tutorial-style prose for reference / how-it-works docs.** When a doc explains how something works (an architecture, an editable surface, a walkthrough), favour **tutorial prose** over dense one-liners: short sentences with **explicit antecedents** (never "the four" — name them), **tables** for parameter / option rosters, **ASCII flow or pseudocode diagrams** for control flow and pipelines, and a **plain-English intro** ("what you're hearing" / "what this does") before each technical block. **Length is not the concern; clarity is.** The shape to mirror is `docs/sound_studio_reference.md` — per-engine case studies, each with a parameter table + an annotated pseudocode walkthrough.
