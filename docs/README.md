...
1. **Hardware** — `hardware/sound_hardware_model.md` — the 6802 sound board you'll emulate
2. **Primitives** — `hardware/synthesis_techniques.md` — the 8 DSP techniques every sound is built from
3. **Catalogues** — `catalogue/{defender,stargate,robotron}_sound_catalogue.md` — every sound's command code + parameters
4. **Pedagogy** — `design/pedagogical_design.md` — *how to explain these sounds* — design principles + UX patterns
5. **Prior art** — `design/sound_studio_reference.md` — what zapspace's Defender Sound Studio teaches
6. **Architecture** — `implementation/explorer_architecture.md` — the 6-phase plan and snapshot schema
7. **Implementation** — `implementation/explorer_implementation.md` — **what's actually built** (Phases 1–6 done; all 12 UX patterns shipped)
8. **Reference audio** — `pipeline/reference_audio_plan.md` — how to obtain WAVs of every effect

If you need only one document: read `implementation/explorer_implementation.md` (it links to everything else as you need it).
...
### `design/` — pedagogy, explainer content, prior art ⚙ *`explainer_cards.md` consumed by `tools/build_explainer_cards.py`*
