...
- ✅ **2026-05-29 Designer Phase 9 — RADIO editor (the fifth and final data-driven engine)**: `$18`, a 16-byte wavetable phase-accumulator. A ~1 h feasibility spike first decoded the routine across all games (findings in `research/findings_designer_feasibility.md` § RADIO): the editable surface is the **16-byte `RADSND` LUT** + one **FREQ** 16-bit immediate (`LDX #imm` at `RADIO_BASE+5`, uniform across games). New headless `engine/radioEdit.ts` (record = `[freq, ...16 LUT bytes]`) + `kind:"radio"` in `customRom.ts` + RADIO detection in `projectFromBin.ts`; a new hybrid `web/designer/radioEditor.ts` (FREQ slider + 16-cell click-to-draw canvas) + populated-list / store / CSS wiring (blue accent). **+20 tests (629 total)**; capture `designer-radio-overview`. **This closes Defender per-engine parity with msarnoff's Defender Sound Studio — all 5 data-driven engines (GWAVE / VARI / LFSR / FNOISE / RADIO) are now editable**, across 3 games, running the actual ROMs. SCREAM and HYPER stay un-editable in both tools (no preset record in the ROM — would need an assembler). See **`designer_implementation.md`** § *RADIO editor — Phase 9*.
...
- ✅ **2026-05-29 Designer/Explore UX polish**: (1) **shortcut tooltips** — every keyed control names its key on hover (`Fire … [Space]`, `Pause … [P]`, speed `[1]`–`[4]`, scrub `[S]`/`[L]`/`[R]`, `[D]`/`[I]`, volume `[↑/↓]`, game `[cycle with G]`; Designer Play `[Space]` / Pause `[P]`), annotated from a single pass in `web/ui/keyboard.ts` so the tooltip can't drift from the binding. New reusable `attrContains` e2e assertion verifies it (on `tut-01-first-sound` + `designer-overview`). (2) **Item list → max 7 rows per column** (`grid-template-rows: repeat(7, auto)`). (3) **Transport moved out of its sticky-bottom position to directly below the item list** (above the parameter editor) — playback sits next to sound selection; the editor + scope follow. (Supersedes the 2026-05-28 redesign entry's "transport sticks to the viewport bottom".) Docs swept (MANUAL + MANUAL_DESIGNER shortcut notes + designer alt-text). 629 tests green; designer captures regenerated.

## Suggested next concrete steps

**All 12 UX patterns from `docs/pedagogical_design.md` are delivered.** The **Sound Designer** edits **all 5 of Williams's data-driven engines** (VARI + GWAVE + LFSR + FNOISE + RADIO) across three games (Defender / Stargate / Robotron) — **per-engine parity with the Defender Sound Studio** — with a "fork-the-game" pre-populated item list, **Open in Explore** for live worklet audition, and a full `.bin` download/upload roundtrip for MAME / real-cabinet use.

**Designer next-up:** the five data-driven engines are complete. Remaining engines (SCREAM / HYPER / ORGAN-pitch) aren't data-authorable — they'd need an in-browser 6800 assembler WSED deliberately doesn't ship. v-future items: Robotron as a VARI engine base (non-linear dispatcher); adding new command codes per engine (dispatcher injection).
