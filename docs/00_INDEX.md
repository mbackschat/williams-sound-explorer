...
- 📋 **2026-05-28 Designer Phase 7 + 8 planned — LFSR + FNOISE editors**: research done (byte-level details in `research/findings_designer_feasibility.md` § LFSR / § FNOISE). LFSR is uniformly inline-immediate in caller code across all three games (LITE / APPEAR / TURBO / Robotron LAUNCH — a virtual record mapped to 2–5 operand bytes per caller). FNOISE is split: **Robotron has a proper 6-byte `FNTAB` data table at `$F785`** (clean indexed authoring), Defender / Stargate have the same parameters inline. After both ship, WSED's engine coverage matches msarnoff's Defender Sound Studio (GWAVE / VARI / LFSR / FNOISE) while spanning 3 games and running the actual ROMs. Plan: `plans/designer-mode.md` § Phase 7 + Phase 8. Scope: ~6 h LFSR + ~7 h FNOISE end-to-end.
