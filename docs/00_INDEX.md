...
- 📋 **2026-05-28 Designer Phase 7 + 8 + 9 planned — LFSR, FNOISE, RADIO editors**: research done for LFSR + FNOISE (byte-level details in `research/findings_designer_feasibility.md` § LFSR / § FNOISE); RADIO still needs a feasibility spike. After all three ship, WSED has Defender per-engine parity with msarnoff's Defender Sound Studio (GWAVE / VARI / LFSR / FNOISE / RADIO — 5 of 5 data-driven engines) while spanning 3 games and running the actual ROMs. SCREAM and HYPER stay un-editable in both tools — they have no preset record in the ROM and would need an in-browser 6800 assembler we deliberately don't ship. Plan: `plans/designer-mode.md` § Phase 7 + Phase 8 + Phase 9. Scope: ~6 h LFSR + ~7 h FNOISE + ~6 h RADIO end-to-end.
